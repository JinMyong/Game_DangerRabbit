...
+ (NSDictionary *) incrementalTagWithInteger:(NSInteger)delta {
	return [NSMutableDictionary dictionaryWithObjectsAndKeys:@"increment", @"operation", [NSNumber numberWithInteger:delta], @"value", nil];
}
@end
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	if(buttonIndex != 1) {
		...
		[pushNotifications removeObjectForKey:[NSNumber numberWithInteger:alertView.tag]];
		return;
	}
	
	NSDictionary *lastPushDict = [pushNotifications objectForKey:[NSNumber numberWithInteger:alertView.tag]];
...
- (BOOL) handlePushReceived:(NSDictionary *)userInfo {
	...
	if(!isPushOnStart && showPushnotificationAlert && msgIsString) {
		...
		[pushNotifications setObject:userInfo forKey:[NSNumber numberWithInteger:internalIndex]];
		[alert show];
		[alert release];
		return YES;
	}
...
- (void) sendBadges: (NSInteger) badge {
	[self performSelectorInBackground:@selector(sendBadgesBackground:) withObject:[NSNumber numberWithInteger:badge]];
}
